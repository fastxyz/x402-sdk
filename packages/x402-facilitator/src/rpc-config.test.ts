...
describe("RPC config overrides", () => {
  ...
  function createRequirement(network: string = "arbitrum-sepolia"): PaymentRequirement {
    return {
      scheme: "exact",
      network,
  ...
  function createPayload(network: string = "arbitrum-sepolia"): PaymentPayload {
    return {
      x402Version: 1,
      scheme: "exact",
      network,
  ...
  it("uses the configured rpcUrl during verification", async () => {
    const result = await verify(createPayload(), createRequirement());
  ...
  it("uses the configured rpcUrl during settlement", async () => {
    const result = await settle(createPayload(), createRequirement(), {
  ...
  it("supports custom EVM networks with custom chain ids", async () => {
    writeFileSync(configPath, JSON.stringify({
      evm: {
        "my-custom-network": {
          chainId: 84532,
    ...
    const result = await verify(
      createPayload("my-custom-network"),
      createRequirement("my-custom-network")
    );
    ...
    const [clientConfig] = vi.mocked(viem.createPublicClient).mock.calls[0];
    expect(clientConfig.transport).toEqual({ url: customRpcUrl });
    expect(clientConfig.chain?.id).toBe(84532);

    const [verifyArgs] = viemMocks.publicClient.verifyTypedData.mock.calls[0];
    expect(verifyArgs.domain.chainId).toBe(84532);
  });
});
